...
infile_lines = open("data.json", "r").read()
...
for image in infile_images:
  if image:
    ...
    print canvas_info_str("image_" + str(count))
    print "</table>"
    print "<br />"
    print "<b>" + utterance + "</b><br />"
    print label
    print "<br /><br />"
...
for image in images_worlds:
  for world in image:
    print "var " + world + " = document.getElementById(\"" + world + "\").getContext(\"2d\")"
    
    desc_formatted = json.dumps(img_desc_def[index][1])
    print "worlds_json = JSON.parse(" + desc_formatted + ")"
    print "draw_objects(" + world + ", worlds_json)"
  index += 1
